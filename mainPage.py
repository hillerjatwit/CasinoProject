...
def login():
    ...
    for user in user_db:
        if user[0] == username and user[1] == password:
            ...
            global current_user
            current_user = User(username, 1000, password)
            show_main_page()
            break
    else:
...
def play_slots():
    ...
    while cont == 1 and bank > 0:
        ...
        def playgame():
            ...
            spin1 = StringVar()
            spin1.set("Spinning...")
            spinning1 = Label(f, text="Spinning", width=25).grid(row=4, column=1)
            spin2 = StringVar()
            spin2.set("Spinning...")
            spinning2 = Label(f, text="Spinning", width=25).grid(row=4, column=2)
            spin3 = StringVar()
            spin3.set("Spinning...")
            spinning3 = Label(f, text="Spinning", width=25).grid(row=4, column=3)
...
craps_button = ttk.Button(main_frame, text="Play Craps", command=lambda: [main_frame.pack_forget(), login_frame.pack_forget(), signup_frame.pack_forget(), play_craps()])
